...
def vec_sum(veclist, D):
    ...
    True
    '''
    return Vec(D, {d:sum([v[d] for v in veclist]) for d in D})
...
def scale_vecs(vecdict):
    ...
    [True, True]
    '''
    return [v * (1.0/k) for k,v in vecdict.items()]
...
def GF2_span(D, S):
    ...
    if len(S) == 0:
        ...
        return {Vec(D, {}), listS[0]}
    else:
        oneless = GF2_span(D, listS[1:])
        withTheOne = {v+listS[0] for v in oneless}
        total = oneless.union(withTheOne)
        return total
