...
def create_voting_dict(strlist):
    ...
    for s in strlist:
        ss = s.split()
        d[ss[0]] = [int(ss[x]) for x in range(3, len(ss))]
...
def most_similar(sen, voting_dict):
    ...
    """
    maxsimilarity = -len(voting_dict[sen])
    name = None
    for k, v in voting_dict.items():
        if k != sen:
            simi = policy_compare(sen, k, voting_dict)
...
def least_similar(sen, voting_dict):
    ...
    Example:
        ...
        'c'
    """
    minsimilarity = len(voting_dict[sen])
    name = None
    for k, v in voting_dict.items():
        if k != sen:
            simi = policy_compare(sen, k, voting_dict)
            if simi < minsimilarity:
                minsimilarity = simi
                name = k
...
votinglist = list(f)
voting_dict = create_voting_dict(votinglist)
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    Example:
        ...
        -0.5
    """
    ss = [policy_compare(sen, sa, voting_dict) for sa in sen_set if sa != sen]
    return float(sum(ss))/len(ss)

def create_voting_dict_1party(strlist, party):
    d = {}
    for s in strlist:
        ss = s.split()
        if ss[1] == party:
...
votinglist = list(f)
voting_dict = create_voting_dict(votinglist)
voting_dict_Democrat = create_voting_dict_1party(votinglist, 'D')

maxsimi = -len(votinglist[0])
name = None
for sen, v in voting_dict.items():
    simi = find_average_similarity(sen, voting_dict_Democrat.keys(), voting_dict)
    if simi > maxsimi:
        maxsimi = simi
        name = sen
    ...
    for s in sen_set:
        ...
        if combined == None:
            ...
            for i in range(len(v)):
                combined[i] = combined[i] + v[i]

    return [float(c)/len(sen_set) for c in combined]

f = open('voting_record_dump109.txt')
votinglist = list(f)
voting_dict = create_voting_dict(votinglist)
voting_dict_Democrat = create_voting_dict_1party(votinglist, 'D')
find_average_record(voting_dict_Democrat.keys(), voting_dict)
...
def bitter_rivals(voting_dict):
    ...
    pair = None
    minsimi = 999999999
    for sena, va in voting_dict.items():
        for senb, vb in voting_dict.items():
            if senb == sena: continue
            simi = sum([a*b for (a,b) in zip(va, vb)])
            if simi < minsimi:
                minsimi = simi
                pair = (sena, senb)
